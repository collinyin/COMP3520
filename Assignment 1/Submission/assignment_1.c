...
void * teacher_routine(void * arg) {
    ...
    while (total_student_counter < N && group_counter) {
        ...
        if (group_counter != 1) {
            ...
            sleep(time_taken);
            pthread_cond_broadcast(&leave_room);
            
            pthread_cond_wait(&last_student, &lock);

            printf("Teacher: Group %d took %f units of time to complete the exercise.\n", group_counter - 1, time_taken);
            printf("______________________________________________________________________________________________________\n");            
        }

        // this is to ensuring the thread prints out correctly
        if (N % K > 0) {
...
void * student_routine(void * arg) {
    ...
    if (N - (K * (group_counter - 1)) < K) {
        ...
        if (last_student_check == 1) {
            ...
            // if the last student is the only student in the group
            if (N - (K * (((struct student *) arg)->groupID - 1)) <= 1) {
                pthread_cond_wait(&leave_room, &lock);
                pthread_cond_signal(&last_student);
            }
        }
        // all students except the last student waits until the teacher tells them that they can leave
        else {
            pthread_cond_wait(&leave_room, &lock);
            printf("Student in group %d: We have completed our exercise and leave the lab now.\n", ((struct student *) arg)->groupID);
            pthread_cond_signal(&last_student);
